...
impl NeoMediaFactory {
    fn new() -> Self {
        ...
        // Without this, each client (go2rtc, ffprobe, health-check probes) creates
        // its own pipeline and camera stream session, exhausting file descriptors
        ...
        // Keep the pipeline alive when clients disconnect instead of tearing it down
        // and rebuilding on every probe/reconnect. Reset mode causes resource churn.
