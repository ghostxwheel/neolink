...
/// Round `n` up to the next power-of-two bucket, clamped to [MIN_BUCKET, MAX_BUCKET].
/// Returns None when `n` exceeds MAX_BUCKET — those go through a one-off allocation.
fn bucket_size_for(n: usize) -> Option<usize> {
    const MIN_BUCKET: usize = 256;
    // 1 MiB upper bound: frames larger than this are unusual; don't try to pool them.
...
/// Acquire (or allocate) a GStreamer buffer for `data`, using power-of-two pools.
///
/// Previously every unique byte-length produced its own pool, so a camera that
/// varies frame sizes continuously (H.264 with traffic) could accumulate thousands
/// of pools, each pre-allocating 8–32 buffers — a steady memory and fragmentation
/// leak over hours/days of operation.  Bucketing collapses those into at most
/// log2(MAX_BUCKET/MIN_BUCKET) ≈ 12 pools per appsrc.
fn acquire_pooled_buffer(
    ...
    if let Some(bucket) = bucket_size_for(needed) {
        let pool = pools.entry(bucket).or_insert_with(|| {
            ...
            // caps=None, size=bucket, min_buffers=8, max_buffers=64
        ...
        // Oversized frame — allocate without pooling to avoid wasting pool slots.
...
fn send_to_appsrc(
    ...
    // In live mode follow the GStreamer live-source design:
    // only push while Playing and a clock is available; timestamp at wall clock.
    if appsrc.is_live() {
    ...
    match appsrc.push_buffer(buf) {
        ...
        Err(gstreamer::FlowError::Flushing) => {
            // The downstream leaky queue discards the oldest frame automatically;
            // we just log and move on so the producer never blocks.
            log::debug!(
                "Appsrc {} is flushing — downstream queue full, frame discarded (live mode)",
                appsrc.name()
            );
            return Ok(());
        }
...
fn make_dbl_queue(name: &str, buffer_size: u32) -> AnyResult<Element> {
    ...
    queue.set_property(
        ...
        std::convert::TryInto::<u64>::try_into(
            tokio::time::Duration::from_secs(1).as_nanos(),
        )
        .unwrap_or(0),
    );
...
fn make_queue(name: &str, buffer_size: u32) -> AnyResult<Element> {
    ...
    queue.set_property("max-size-bytes", buffer_size);
    queue.set_property("max-size-buffers", 0u32);
    // Live-stream mode: cap at exactly one I-frame period (1 s at 25 fps, 1x multiplier).
    // When the queue is full we drop the OLDEST frame (leaky=downstream) so the output
    // always reflects the most recent camera data rather than buffering into the past.
    queue.set_property(
        "max-size-time",
        std::convert::TryInto::<u64>::try_into(
            tokio::time::Duration::from_secs(1).as_nanos(),
...
fn buffer_size(bitrate: u32) -> u32 {
    // Size the appsrc buffer to hold exactly 1 second of compressed data
    // (matching the 1x I-frame multiplier assumption: one keyframe per second at 25 fps).
    // bitrate is stored in bits/sec; /8 converts to bytes/sec = 1 s of data.
    // Floor at 256 KiB so low-bitrate or unknown streams still have a workable buffer.
    std::cmp::max(bitrate / 8u32, 256u32 * 1024u32)
}
